 ...
 * Created on Sept 27th, 2021, 5:55 PM
 */
...
int *mode(const int *array,int arySize){
    ...
    int freq;                //Keep count of the max number of matches
    int maxFreq=0;             //Keeps handy maximum frequency
    ...
    for(int i=1;i<arySize;i++){
        if(ary[i]==ary[i-1]){
            freq++;
            if(freq==maxFreq){modeAry[j+2]=ary[i-1],freq=1,j++;}
        }else{
            if(freq==maxFreq){modeAry[j+2]=ary[i-1],j++;}
            freq=1;
        }
    }
